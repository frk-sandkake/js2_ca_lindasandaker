                  ...
                  <li class="m-4 col-span-1 p-4 border-fuchsia-700 border-2 border-b-4 rounded shadow focus-within:ring-2 focus-within:ring-inset focus-within:ring-teal-600">
                    <div class="grid grid-cols-6 pb-4">
                        <div role="img" class="col-span-6 py-4">
                            ...
                            <span role="presentation" class="font-semibold text-orange-50">Users_Name_Here</span>
                        </div>                 
                        <a href="/single-post.html?post_id=${id}" class="col-span-5">
                            <h3 class="pl-4 capitalize text-2xl font-serif text-cyan-300 hover:text-cyan-900">${title}</h3>
                        </a>            
                        <time datetime="2021-01-27T16:35" class="col-span-1 text-xs text-neutral-300 whitespace-nowrap">${createdWhen}</time>
                        <p class="my-2 p-4 bg-orange-50 rounded-md font-medium col-span-6">${body}</p>
                    </div>
                  </li>
              `;
      })
...
searchBar.addEventListener('keyup', (e) => {
  ...
  const postsFound = posts.filter((post) => post.title.toLowerCase().includes(searchText) || post.body.toLowerCase().includes(searchText));
